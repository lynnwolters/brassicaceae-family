<script>
        ...
        for (var e = [], r = {}, s = a.split(/\s*(;|\(|\)|,|:)\s*/), t = 0; t < s.length; t++) {
            ...
            switch (n) {
                ...
                default:
                    ...
                    if (r.sample && !r.speciesName) {
                        const sample = r.sample;
                        const matchingSpecies = jsonData.find(item => item.SAMPLE == sample);
                        if (matchingSpecies) {
                            r.speciesName = matchingSpecies.SPECIES_NAME_PRINT;
                        }
                    }
            }
        }
    ...
    onMount(async () => {
        await fetchJsonData(); 
        const parsedData = parseData(newickData, jsonData);
        const svgContainer = document.querySelector("#tree-of-life");
        if (svgContainer) {
            svg = createTreeOfLife(parsedData);
    ...
    let svg;
    const svgWidth = 1400;
    const svgOuterRadius = svgWidth / 2;
    const svgInnerRadius = svgOuterRadius - 170;
    ...
    const updateSuggestions = () => {
        ...
        datalist.innerHTML = "";
        filteredSpeciesData.forEach((item) => {
            const option = document.createElement("option");
            option.value = item.SPECIES_NAME_PRINT;
    ...
    const createTreeOfLife = (data) => {
        ...
        setRadius(root, (root.data.length = 0), svgInnerRadius / maxLength(root));
    ...
    const linkExtensionConstant = (d) => {
        return linkStep(d.target.x, d.target.y, d.target.x, svgInnerRadius);
    };

    const linkStep = (startAngle, startRadius, endAngle, endRadius) => {
        const c0 = Math.cos(
            (startAngle = ((startAngle - 90) / 180) * Math.PI)
        );
...
<header>
    <ul class="header-left">
        ...
        <li>
            <form>
                <input bind:value={searchSpecies} type="text" placeholder="Search..." on:input={handleSearchFilter} list="species-list" />
        ...
        <div class="popup">
            <div class="popup-content">
                ...
                <div class="popup-content-main">
                    <div class="popup-content-main-labels">
                        <button>
                            <div>
                                <img src="images/relation-label.svg" alt="genus">
                            </div>
                            <div>
                                <p>Genus</p>
                            </div>
                        ...
                        <button>
                            <div>
                                <img src="images/kind.svg" alt="kind">
                            </div>
                            <div>
                                <p>Kind</p>
                            </div>
                    ...
                    <div class="popup-content-main-info">
                        <h2>{selectedNode.data.speciesName}</h2>
                        <p>Arabis scabra, the Bristol rockcress, is a perennial flowering plant in the family Brassicaceae, native to the Bristol region, including the Avon Gorge.[2] It grows in shallow soils, scree and rocky ledges.[2] It has been introduced to other locations in England, with populations usually being short-lived, with the exception of Combwich, where it can still be found.[2]</p>
                        <button>
                            iNaturalist
                            <div>
                                <img src="images/iNaturalist.svg" alt="iNaturalist">
                            </div>
                        </button>
                    </div>
                    <div class="popup-content-main-relation">
                        <h3>Relation</h3>
                        <p>De blablabla is verwant met blablabla, blablabla, blablabla, blablabla, blablabla, blablabla en blablabla.</p>
                        <button>
                            <div>
                                <img src="images/relation.svg" alt="relation">
                            </div>
...
<style>
    ...
    .popup-content-header button:nth-of-type(2) {
        background-color: var(--color-7);
    }

    .popup-content-header button img {
        width: 50%;
    }
    ...
    .popup-content-main-labels {
        display: flex;
        flex-wrap: wrap;
    }

    .popup-content-main-labels button {
        width: fit-content;
        height: 2.5em;
    ...
    .popup-content-main-labels button:nth-of-type(1) {
        border: solid .1em var(--color-5);

        color: var(--color-5)
    }

    .popup-content-main-labels button:nth-of-type(2) {
        border: solid .1em var(--color-4);

        color: var(--color-4)
    }

    .popup-content-main-labels button:nth-of-type(3) {
        border: solid .1em var(--color-6);

        color: var(--color-6)
    }

    .popup-content-main-labels button:nth-of-type(4) {
        border: solid .1em var(--color-7);

        color: var(--color-7)
    }

    .popup-content-main-labels button div:nth-of-type(1) {
        aspect-ratio: 1/1;
        height: 100%;
    ...
    .popup-content-main-labels button div:nth-of-type(1) img {
        width: 50%;
    }

    .popup-content-main-labels button div:nth-of-type(2) {
        display: flex;
        justify-content: end;
    }

    .popup-content-main-info h2 {
        margin-bottom: .5em;

        font-size: 1.5em;
    }

    .popup-content-main-info p {
        margin-bottom: 1em;
    ...
    .popup-content-main-info button {
        width: fit-content;
        height: 2.5em;
    ...
    .popup-content-main-info button div {
        aspect-ratio: 1/1;
        height: 100%;
    ...
    .popup-content-main-info button div img {
        width: 50%;
    }

    .popup-content-main-relation h3 {
        margin-bottom: .5em;

        font-size: 1.4em;
    }

    .popup-content-main-relation p {
        margin-bottom: 1em;
    ...
    .popup-content-main-relation button {
        width: fit-content;
        height: 2.5em;
    ...
    .popup-content-main-relation button div {
        aspect-ratio: 1/1;
        height: 100%;
    ...
    .popup-content-main-relation button div img {
        width: 45%;
    }

    .popup-content-main-relation button:hover {
        background-color: var(--color-4);
    }
